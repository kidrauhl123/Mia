...
(function (global) {
  ...
  function renderBotCard(args) {
    ...
    const avatar = resolveCardAvatar({
      id: avatarId,
      displayName: name,
      avatarImage: local ? local.avatarImage : (identityAvatar.image || member?.bot_avatar_image || ""),
      avatarCrop: local ? local.avatarCrop : (identityAvatar.crop || member?.bot_avatar_crop || null)
    });
