...
`dist:mac`、`dist:mac:intel` 和 Windows release 内置 packaged-Core gate：打包前运行 `scripts/prepare-mia-core-rs.js`，把 release Rust Core 和目标架构的 Claude/Codex ACP 放进 `resources/bundled-mia-core/<platform>-<arch>/`；打包后运行 `scripts/verify-packaged-mia-core.js`，确认 Core、两个 ACP manifest 及其入口文件齐全，并直接启动 Rust Core 等待 `/health` 成功响应。这个 gate 的目的不是“看文件在不在”，而是阻断这类真实故障：
...
`npm run dist:mac` 末尾用 `scripts/create-mac-dmg.js` 生成带样式的安装窗口，规格是固定的（改动需同步更新这里）：
...
- `docs/releases/<version>.md` 面向普通用户，不是内部变更日志。**更新说明必须极简：优先 1–3 条、每条一句，只保留最重要的用户可感知变化或修复；宁可遗漏次要内容、写得过于简略，也不要写成详细功能清单或变更日志。** 避免服务器、脚本、临时文件名、内部架构名、发包流程等实现细节；也不要写“发布了新安装包”“更新到某版本”这类用户已经知道的空话。
...
- 客户端检查到新版本后是强制更新：主界面会被更新遮罩锁定，下载进度来自 `download-progress`，下载完成后自动进入安装并重启。进入安装态时只能锁页面交互，不能继续用 native `setClosable(false)` 锁窗口；macOS 的 ShipIt/Squirrel.Mac 交接由 `electron-updater` 原生完成，应用只调用一次 `quitAndInstall`，不得再叠加 `app.quit()` watchdog 或超时兜底，否则可能在替换前中断安装。
...
`mia-jms-deploy` 是本机 SSH 部署别名。JumpServer 的主机、资产路由、账号和认证方式属于会变动的基础设施配置，不在仓库中硬编码；新机器应通过受控的运维流程获取该别名及访问授权，不能从历史文档复制内网地址或登录信息。

部署前只校验别名是否指向预期通道，且不要把命令输出、密码或资产路由粘贴进工单和日志：

```bash
ssh -G mia-jms-deploy | awk '$1 == "hostname" || $1 == "port" { print }'
```

#### 一条命令部署（无需手动输密码）

`scripts/deploy-cloud-jms.sh` 使用已由访问流程配置好的本机 SSH 别名和认证方式；若该机器的授权尚未配置，请联系基础设施负责人，不要把密码、私钥或资产直连用户名写入仓库、Shell 历史或文档。授权完成后部署就是一条命令：

```bash
bash scripts/deploy-cloud-jms.sh
```

该 wrapper 会建立或复用跳板连接，再带上腾讯云镜像源调用底层部署脚本。可叠加常规开关，例如 `MIA_DEPLOY_DRY_RUN=1`、`MIA_DEPLOY_SKIP_LOCAL_TESTS=1`、`MIA_INSTALL_SKIP_HERMES_IMAGE_BUILD=1`。认证材料应由系统钥匙串、SSH agent 或团队认可的凭据管理方式提供，优先使用密钥认证。
