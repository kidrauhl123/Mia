...
- **managed-model runtime** — stubbed (graceful: Mia-managed-model bots fall through to their own runtimeConfig provider/model; nothing crashes). It is NOT just a model name (the run payload already carries `model` per-turn) — it writes the Mia cloud model-proxy PROVIDER (base_url + token + apiMode) into Hermes `config.yaml`. **AION pattern (studied in AionCore):** the backend OWNS provider/model config (per-conversation in its DB) and the frontend never writes it — see `aionui-system/src/provider.rs` (provider CRUD+encryption) + `aionui-ai-agent/.../agent_reconcile.rs` (per-run `set_model`). For Mia this means a clean follow-up slice: **Core owns Hermes engine config writing** (Core is the single-owner daemon, so it should write `config.yaml` for the managed provider; the GUI must stop calling `startEngine`/`writeRuntimeConfig`). Deferred because it touches the GUI's existing engine wiring (dual-write risk) and is broader than the narrow managed-model degradation it fixes — its own engine-lifecycle-ownership slice.
- **Engine lifecycle ownership (next big slice, AION-aligned)** — AionCore's `aionui-runtime` spawns + supervises the agent runtime subprocess (bundled-first resolution, process groups, `--parent-pid` auto-exit) and is the sole long-lived backend. Mia's Hermes engine is currently started by the Electron side / a separate `ai.mia.hermes.gateway` launchd service; the node Core only discovers its endpoint (config.yaml port + key — done). Full AION parity = Core owns the engine subprocess lifecycle too. Tracked as the next slice; not required for the daemon to function (the gateway launchd service / GUI start the engine; Core reaches it).
