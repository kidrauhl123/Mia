...
Mia 是通过 Mia 发起的 Bot 对话中长期关系记忆的唯一 owner。

| 内容 | Owner |
| --- | --- |
| 当前对话历史、上下文压缩、原生 session resume | 原生引擎 |
| AGENTS.md、CLAUDE.md、SOUL.md、项目规则和技能 | 原生引擎 |
| 用户公共画像、Bot 专属关系记忆 | Mia |
| 原生引擎 auto memory | 在 Mia runtime 内关闭 |

关闭原生长期记忆只作用于 Mia 启动的进程或 session。Mia 不修改用户的全局配置，也不维护私有 Codex/Hermes home。用户离开 Mia 后直接运行原生引擎时，原生记忆保持原样。
...
4. Mia 在自身 runtime 内关闭原生引擎记忆，避免形成第二个 owner。
...
新原生 session 创建时，Mia 读取 user 与当前 Bot memory，渲染两个有明确边界的冻结块。每块显示名称、使用比例和字符数，条目以 § 分隔。空文档可以省略条目正文，但仍允许工具工作。
...
冻结快照在原生 session 创建后、第一条用户消息交给 Agent 前装配一次。运输层优先使用引擎支持的 session 初始 instruction；统一 ACP 路径无法提供独立 system/developer 槽位时，使用现有 initialPromptPrefix 在第一条 prompt 前追加一次。
...
策略上线前已经创建的原生 session 可能含有原生引擎记忆，无法从现有上下文中安全移除。新隔离策略只保证新建原生 session；不得静默重置正在使用的旧 session。用户创建新对话后获得完整新语义。

## Memory 工具

Mia 只向 Agent 暴露一个 memory 工具。MCP 运输层可以按引擎规则给工具加 server namespace，但工具 schema 和行为保持一致。
...
Mia 创建 Hermes Agent 时使用上游运行级 skip_memory=true，使 Hermes 不读取或写入原生 MEMORY.md、USER.md，也不暴露原生 memory 工具。保留 AGENTS.md、SOUL.md、技能、工具和原生 session。

本地 Hermes 必须先探测实际版本或能力；云端镜像必须 pin 到支持该参数的版本。无论 Mia memory 是否启用，如果当前 Hermes runtime 不能安全关闭原生记忆，启动新 session 都应返回明确的不兼容错误，不能静默回退到双记忆，也不能修改用户全局 config.yaml。

### Codex

Mia 启动 Codex runtime 时使用单次运行配置覆盖：
...
Mia 启动 Claude Code runtime 时设置：
...
即使用户关闭 Mia memory，Mia runtime 内仍关闭原生长期记忆。关闭代表通过 Mia 的 Bot 没有长期关系记忆，而不是悄悄切换到引擎自带记忆。
...
- 诊断场景可以显示只读容量和启用状态，但不能形成第二个写 owner。
...
- fresh session 恰好注入一份 user 与一份 memory。
- 同一 session 第二回合不注入。
- load/resume 成功时不注入。
...
- Hermes Agent 以 skip_memory 启动，不加载原生 MEMORY.md/USER.md，不暴露原生 memory 工具。
- Codex Mia 进程获得两项单次 memories=false 覆盖，用户 ~/.codex 配置文件不改变。
- Claude Code Mia 进程设置 CLAUDE_CODE_DISABLE_AUTO_MEMORY=1，同时仍加载项目 CLAUDE.md。
- 三套引擎继续使用真实原生 session resume。
- Mia 不写 ~/.codex、~/.hermes、~/.claude 的原生记忆。
- 不支持隔离能力的 Hermes 版本明确失败，不静默形成双记忆。
...
- Agent 可见的记忆模型只有两个有界文本 target 和一个 memory 工具。
- Mia 是长期关系记忆唯一 owner。
- 原生长期记忆在 Mia runtime 内关闭，用户全局原生环境不被修改。
- 记忆只在 fresh 原生 session 注入一次。
- Bot 身份、引擎切换和 Cloud sync 不改变记忆归属。
- 当前重型搜索、provider extraction 和五工具模型不再参与 Agent runtime。
- 所有失败都明确、可诊断，且不产生静默截断、双写或跨 Bot 泄漏。
