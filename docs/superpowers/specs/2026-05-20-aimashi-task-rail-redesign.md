...
把现有 rail 第 4 项「工作台」换成「任务」，作为 aimashi 的"定时 / 日程"一级入口。任务通过日常对话创建，由 daemon 调度，输出落回原 fellow 会话；任务页本身承担调度可视化、微操编辑、历史回看。
...
aimashi 的核心宗旨是"对话即一切，降低用户心智"。任务的所有创建、复杂修改都走对话；任务页只承担**可视化**和**微操**。
...
- run 的"输出"不重复存：消息本体在 `aimashi-sessions.json` 里，靠 `outputMessageId` 引用
...
调 schedule.create(title, trigger, prompt) tool
  ↓
daemon 写入 aimashi-tasks.json，返回 taskId
  ↓
fellow 在对话里输出一条确认卡片：
  ┌──────────────────────────────────────┐
  │ 📅 已创建定时任务                       │
  │ "巡检 Hermes 日志"                     │
  │ 每天 09:00 · Aimashi · 下次 明天 09:00 │
  │ [查看] [改时间] [取消任务]              │
  └──────────────────────────────────────┘
```
...
复用 runRemoteChatRequest({ sessionId, prompt, meta: { taskId, taskRunId } }, eventSink)
  ↓
adapter 流式写 message 到 aimashi-sessions.json
...
来源会话: "Hermes 后台" · Aimashi · 5/19 16:23  [打开 →]
...
aimashi 已有 daemon 模式（`--daemon` / `AIMASHI_DAEMON=1`），通过 macOS LaunchAgent `ai.aimashi.daemon` 自动启动，已有 HTTP 控制服务器 + relay 客户端。
...
GUI **不直接读 `aimashi-tasks.json`**，避免与 daemon 写入竞争。

### 7.4 MCP server `aimashi-scheduler`
...
新文件 `~/Library/Application Support/Aimashi/aimashi-tasks.json`（和 `aimashi-sessions.json` 同目录），原子写入（沿用既有模式）。
...
- `src/main/tasks-store.js` —— `aimashi-tasks.json` 读写（daemon 端）
