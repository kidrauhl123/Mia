...
function buildHandoff({
  ...
  return [
    ...
    "```",
    "",
    "After the desktop bridge is logged into the same dedicated WeChat smoke account, run the end-to-end bridge smoke:",
    "```bash",
    "cd /tmp",
    ...
    "",
    "If the operator is using the standalone local Agent bridge instead of the desktop app, start it from a full Mia project checkout on the bridge machine with the smoke account token first. This command is not run from the extracted Cloud release directory:",
    "```bash",
    "cd /path/to/mia",
...
function buildTransferReadme({ publicUrl, expectedCommit, expectedBuiltAt }) {
  return [
    ...
    "```",
    "",
    "Validate the fixed smoke account token before bridge-required e2e:",
    "",
    "```bash",
    "cd /tmp",
    "tar -xzf mia-cloud-release.tgz mia-cloud-release/prepare-cloud-smoke-account.js mia-cloud-release/smoke-cloud.js",
