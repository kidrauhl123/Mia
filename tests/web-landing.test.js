...
test("web root is a promo landing page with download and app entry points", () => {
  ...
  assert.match(html, /不同 AI，处理不同事/);
  assert.match(html, /学习、创作、求职、日程和灵感整理/);
  assert.match(html, /href="assets\/mia\.css\?v=20260625-realchat"/);
  assert.match(html, /href="assets\/mia-gradient\.css\?v=20260625-realchat"/);
  assert.match(html, /href="assets\/mia-scroll\.css\?v=20260625-realchat"/);
  assert.match(html, /src="assets\/mia\.js\?v=20260625-realchat"/);
  assert.match(html, /src="assets\/mia-scroll\.js\?v=20260625-realchat"/);
  ...
  assert.match(js, /帮我把课程资料整理成 5 分钟展示稿/);
  assert.doesNotMatch(js, /展示准备包/);
  assert.doesNotMatch(js, /mw-result/);
  assert.match(js, /讲稿结构、待办和提醒/);
  assert.match(js, /下一步只剩三件事/);
