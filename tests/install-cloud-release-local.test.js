...
function createFixtureRelease({ badHash = false } = {}) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "aimashi-install-verify-"));
  const releaseRoot = path.join(tempDir, "aimashi-cloud-release");
  const files = {
    "api/server.js": "module.exports = {};\n",
    "api/package.json": "{\"name\":\"aimashi-cloud-api\"}\n",
    "web/index.html": "<!doctype html><title>Aimashi Web</title>\n",
  ...
  const archive = path.join(tempDir, "aimashi-cloud-release.tgz");
  childProcess.execFileSync("tar", ["-czf", archive, "-C", tempDir, "aimashi-cloud-release"]);
  fs.writeFileSync(`${archive}.sha256`, `${sha256File(archive)}  aimashi-cloud-release.tgz\n`);
...
test("local installer verify-only accepts a release archive with matching manifest hashes", () => {
  ...
  try {
    const result = childProcess.spawnSync("bash", [installer, archive], {
      cwd: root,
      env: { ...process.env, AIMASHI_INSTALL_VERIFY_ONLY: "1" },
      encoding: "utf8"
    });
    assert.equal(result.status, 0, result.stderr || result.stdout);
    assert.match(result.stdout, /Release archive checksum OK:/);
    assert.match(result.stdout, /Aimashi Cloud local installer verify-only completed:/);
  } finally {
...
test("package exposes a safe verify-only install command", () => {
  assert.equal(
    ...
    "AIMASHI_INSTALL_VERIFY_ONLY=1 bash scripts/install-cloud-release-local.sh dist/aimashi-cloud-release.tgz"
  );
});
