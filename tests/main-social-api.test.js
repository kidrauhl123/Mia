...
test("cancelConversationRun sends POST to the cloud run cancellation route", async () => {
  const seen = [];
  const ctx = await spawnFakeCloud((req, res) => {
    let body = "";
    req.on("data", (chunk) => { body += chunk; });
    req.on("end", () => {
      seen.push({ method: req.method, url: req.url, body });
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({ ok: true, status: "cancelling" }));
    });
  });
  try {
    const api = createSocialApi({
    ...
    const result = await api.cancelConversationRun("botc_u_1_mia", "car_run_1");
    assert.equal(result.ok, true);
    assert.equal(seen[0].method, "POST");
    assert.equal(seen[0].url, "/api/conversations/botc_u_1_mia/runs/car_run_1/cancel");
