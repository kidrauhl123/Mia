...
test("settings exposes optional update choices through the preload bridge", () => {
  const htmlSource = fs.readFileSync(path.join(root, "src/renderer/index.html"), "utf8");
  const appSource = fs.readFileSync(path.join(root, "src/renderer/app.js"), "utf8");
  const updateSource = fs.readFileSync(path.join(root, "src/renderer/app-update.js"), "utf8");
  const preloadSource = fs.readFileSync(path.join(root, "src/preload.js"), "utf8");
  const mainSource = fs.readFileSync(path.join(root, "src/main.js"), "utf8");
  ...
  assert.match(htmlSource, /id="appUpdatePanel"/);
  assert.match(htmlSource, /id="appUpdateReleaseNotes"/);
  assert.match(htmlSource, /id="appUpdateActions"/);
  assert.match(htmlSource, /id="appUpdateDefer"[^>]*>暂不更新<\/button>/);
  assert.match(htmlSource, /id="appUpdateInstall"[^>]*>立即更新<\/button>/);
  assert.match(htmlSource, /id="appUpdateProgressFill"/);
  assert.match(htmlSource, /app-update\.js[\s\S]*app\.js/);
  assert.match(appSource, /window\.miaAppUpdate\?\.initAppUpdate\(\{ els, api: window\.mia, setText \}\)/);
  assert.match(updateSource, /api\.checkForUpdates\(\)/);
  assert.match(updateSource, /api\.deferAppUpdate\(\)/);
  assert.match(updateSource, /api\.downloadAppUpdate\(\)/);
  assert.match(updateSource, /\["error", "not-available", "disabled", "deferred"\]\.includes\(status\)/);
  assert.match(updateSource, /payload\.downloaded \? "立即安装" : "立即更新"/);
  assert.match(updateSource, /els\.appUpdatePanel\?\.addEventListener\("pointerdown", beginPanelDrag\)/);
  assert.match(updateSource, /panel\.setPointerCapture\?\.\(event\.pointerId\)/);
  assert.match(updateSource, /panel\.releasePointerCapture\?\.\(drag\.pointerId\)/);
  assert.match(updateSource, /function offsetWithinViewport\(x, y, geometry\)/);
  assert.match(updateSource, /resetPanelPosition\(\)/);
  assert.match(updateSource, /api\.onUpdateEvent\?\.\(\(payload\) => handleEvent/);
  assert.match(updateSource, /function releaseNoteLines\(payload = \{\}\)/);
  assert.match(updateSource, /function renderReleaseNotes\(payload = \{\}\)/);
  assert.match(updateSource, /item\.textContent = note/);
  assert.doesNotMatch(updateSource, /setAppShellUpdateLocked/);
  assert.doesNotMatch(updateSource, /\.inert\s*=/);
  ...
  assert.match(preloadSource, /downloadAppUpdate:\s*\(\)\s*=>\s*ipcRenderer\.invoke\(IpcChannel\.UpdateDownload\)/);
  assert.match(preloadSource, /deferAppUpdate:\s*\(\)\s*=>\s*ipcRenderer\.invoke\(IpcChannel\.UpdateDefer\)/);
...
test("desktop window controls use frameless Windows chrome off macOS", () => {
  ...
  assert.match(titleBarSource, /const WINDOWS_LIGHT_TITLE_BAR_COLOR = "#ffffff";/);
  ...
  assert.doesNotMatch(css, /--win-panel-border/);
  assert.doesNotMatch(css, /body\.platform-win32 \.window-controls::after/);
  ...
  assert.match(css, /--win-workspace-corner-radius:\s*16px;/);
  assert.match(css, /--workspace-top-control-offset:\s*18px;/);
  assert.doesNotMatch(css, /--win-workspace-border/);
  assert.match(css, /--win-workspace-shadow:\s*0 0 0 1px rgba\(16, 20, 39, 0\.075\), 0 3px 14px rgba\(16, 20, 39, 0\.065\);/);
  assert.match(css, /--win-titlebar-bg:\s*#ffffff;/);
  assert.match(css, /--win-rail-column-width:\s*70px;/);
  assert.match(css, /--win-rail-bg:\s*#ffffff;/);
  assert.match(css, /--win-sidebar-bg:\s*#ffffff;/);
  ...
  assert.match(css, /body\.platform-win32 \.app-shell:not\(\[data-nav-layout="sidebar-bottom"\]\)\s*\{[\s\S]*?background:\s*var\(--surface\);/);
  assert.match(css, /body\.platform-win32 \.app-shell:not\(\[data-nav-layout="sidebar-bottom"\]\)\[data-active-view="contacts"\],[\s\S]*?body\.platform-win32 \.app-shell:not\(\[data-nav-layout="sidebar-bottom"\]\)\[data-active-view="bot-store"\]\s*\{[\s\S]*?background:\s*var\(--surface\);/);
  assert.match(css, /body\.platform-win32 \.app-shell:not\(\[data-nav-layout="sidebar-bottom"\]\)\[data-active-view="chat"\]\s*>\s*#chatView,[\s\S]*?body\.platform-win32 \.app-shell:not\(\[data-nav-layout="sidebar-bottom"\]\)\[data-active-view="contacts"\]\s*>\s*#contactsView,[\s\S]*?body\.platform-win32 \.app-shell:not\(\[data-nav-layout="sidebar-bottom"\]\)\[data-active-view="bot-store"\]\s*>\s*#botStoreView,[\s\S]*?body\.platform-win32 \.app-shell:not\(\[data-nav-layout="sidebar-bottom"\]\)\[data-active-view="skills"\]\s*>\s*#skillsView,[\s\S]*?body\.platform-win32 \.app-shell:not\(\[data-nav-layout="sidebar-bottom"\]\)\[data-active-view="tasks"\]\s*>\s*#tasksView\s*\{[\s\S]*?position:\s*relative;[\s\S]*?z-index:\s*62;[\s\S]*?margin:\s*0 8px 8px 0;[\s\S]*?border-radius:\s*var\(--win-workspace-corner-radius\);[\s\S]*?background:\s*var\(--workspace-floor\);[\s\S]*?box-shadow:\s*var\(--win-workspace-shadow\);/);
  assert.doesNotMatch(css, /body\.platform-win32 \.app-shell::before/);
  ...
  assert.match(css, /body\.platform-win32 \.nav-rail\s*\{[\s\S]*?margin:\s*calc\(-1 \* var\(--win-titlebar-height\)\) 0 0;[\s\S]*?padding:\s*calc\(var\(--win-titlebar-height\) \+ 8px\) 0 12px;[\s\S]*?border-radius:\s*0;[\s\S]*?background:\s*var\(--win-rail-bg\);[\s\S]*?box-shadow:\s*none;[\s\S]*?backdrop-filter:\s*none;/);
  ...
  assert.match(css, /body\.platform-win32 \.app-shell:not\(\[data-nav-layout="sidebar-bottom"\]\) \.settings-tabs\s*\{[\s\S]*?width:\s*100%;[\s\S]*?border-radius:\s*0;[\s\S]*?background:\s*var\(--win-sidebar-bg\);[\s\S]*?box-shadow:\s*none;[\s\S]*?backdrop-filter:\s*none;/);
  assert.match(css, /body\.platform-win32 \.app-shell\[data-nav-layout="sidebar-bottom"\]\[data-active-view="settings"\] \.settings-sidebar\s*\{[\s\S]*?margin:\s*0;[\s\S]*?border-radius:\s*0;[\s\S]*?background:\s*var\(--win-sidebar-bg\);[\s\S]*?box-shadow:\s*none;[\s\S]*?backdrop-filter:\s*none;/);
  assert.match(css, /body\.platform-win32 \.app-shell\[data-nav-layout="sidebar-bottom"\]\[data-active-view="settings"\] \.sidebar-bottom-nav\s*\{[\s\S]*?left:\s*0;[\s\S]*?bottom:\s*0;[\s\S]*?width:\s*var\(--sidebar-width\);[\s\S]*?border-radius:\s*0;[\s\S]*?background:\s*var\(--win-sidebar-bg\);[\s\S]*?box-shadow:\s*none;[\s\S]*?backdrop-filter:\s*none;/);
  assert.match(css, /body\.platform-win32 \[class~="sidebar-tools"\]\s*\{[\s\S]*?background:\s*var\(--win-sidebar-bg\);/);
  assert.match(css, /body\.platform-win32 \.persona\s*\{[\s\S]*?width:\s*calc\(100% - 12px\);[\s\S]*?margin:\s*0 6px;[\s\S]*?border-radius:\s*8px;/);
  assert.match(skillsCss, /\.skills-topbar\s*\{[\s\S]*?top:\s*var\(--workspace-top-control-offset\);/);
  assert.match(botStoreCss, /body\.platform-win32 \.app-shell:not\(\[data-nav-layout="sidebar-bottom"\]\)\[data-active-view="contacts"\] \.discover-top-bar,[\s\S]*?body\.platform-win32 \.app-shell:not\(\[data-nav-layout="sidebar-bottom"\]\)\[data-active-view="bot-store"\] \.discover-top-bar\s*\{[\s\S]*?top:\s*calc\(var\(--win-titlebar-height\) \+ var\(--workspace-top-control-offset\)\);[\s\S]*?z-index:\s*64;[\s\S]*?height:\s*auto;[\s\S]*?padding:\s*0;/);
  assert.match(botStoreCss, /body\.platform-win32 \.app-shell:not\(\[data-nav-layout="sidebar-bottom"\]\)\[data-active-view="contacts"\] \.contacts-sidebar\s*\{[\s\S]*?margin:\s*58px 8px 10px 0;[\s\S]*?border:\s*0;[\s\S]*?border-radius:\s*8px;[\s\S]*?box-shadow:\s*var\(--rail-expanded-shadow\);/);
  assert.match(botStoreCss, /body\.platform-win32 \.app-shell:not\(\[data-nav-layout="sidebar-bottom"\]\)\[data-active-view="bot-store"\] \.explore-sidebar\s*\{\s*margin-top:\s*0;/);
...
test("sidebar-bottom navigation mode keeps the rail path and exposes four primary entries", () => {
  ...
  assert.doesNotMatch(appSource, /navLayoutChoices|data-nav-layout-choice/);
...
test("chat header is a floating card layer rather than a layout topbar", () => {
  ...
  assert.match(styleSource, /@container\s+chat-view\s+\(max-width:\s*560px\)\s*\{[\s\S]*?#chatView \.session-trigger\s*\{[\s\S]*?grid-template-columns:\s*16px;[\s\S]*?justify-content:\s*center;[\s\S]*?width:\s*38px;[\s\S]*?#chatView \.session-trigger \.current-session-title\s*\{[\s\S]*?display:\s*none;/);
...
test("composer runtime controls hide values Core did not observe", () => {
  ...
  assert.match(syncControls, /const cloudPermissionsManaged = usesManagedCloudAgentPermissions\(controlContext\);/);
  assert.match(syncControls, /const permissionEntries = cloudPermissionsManaged[\s\S]*?: runtimeControlArray\(options\?\.permissionOptions\);/);
  assert.match(appSource, /function usesManagedCloudAgentPermissions\(context = activeBotRuntimeControlContext\(\)\)/);
  assert.match(appSource, /if \(usesManagedCloudAgentPermissions\(\)\) return false;/);
  ...
  assert.match(contactControls, /runtimeKind === "cloud-claude-code"[\s\S]*?: runtimeControlArray\(optionsPayload\?\.permissionOptions\);/);
...
test("bot creation dialog combines runtime location and agent engine into one grouped selector", () => {
  ...
  assert.match(dialogViewSource, /<select\s+value=\{dialog\.runtimeValue\}/);
