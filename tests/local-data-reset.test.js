...
function makeHarness(options = {}) {
  ...
  const { runtimePaths } = createRuntimePaths({
    ...
    MIA_CORE_SERVICE_LABEL: "ai.mia.core",
...
test("prelaunch local data reset clears old desktop state before settings load", () => {
  ...
  const apiKeyPath = runtime.apiKey || runtime.apiServerKey;
  const launchCalls = [];
  ...
  assert.equal(fs.existsSync(apiKeyPath), false);
