...
test("server-local installer restores data backups during rollback", () => {
  ...
  assert.match(source, /AGENT_MODE="\$\{MIA_CLOUD_AGENT_MODE:-claude-code\}"/);
  assert.match(source, /AGENT_PYTHON_VENV="\$\{MIA_CLOUD_AGENT_PYTHON_VENV:-\/opt\/mia-agent-runtime\/python\}"/);
  assert.match(source, /AGENT_PYTHON_BIN="\$\{MIA_CLOUD_AGENT_PYTHON_BIN:-python3\.12\}"/);
  assert.match(source, /AGENT_PYTHON_PACKAGES="\$\{MIA_CLOUD_AGENT_PYTHON_PACKAGES:-python-pptx python-docx openpyxl xlsxwriter pandas numpy matplotlib pillow reportlab pypdf requests beautifulsoup4 lxml markdown\}"/);
  assert.match(source, /CLAUDE_CODE_BASE_URL="\$\{MIA_CLOUD_CLAUDE_CODE_BASE_URL:-\$\{MIA_DEEPSEEK_ANTHROPIC_BASE_URL:-https:\/\/api\.deepseek\.com\/anthropic\}\}"/);
  assert.doesNotMatch(source, /Environment=MIA_CLOUD_CLAUDE_CODE_API_KEY=/);
  ...
  assert.doesNotMatch(source, /is_legacy_hermes_agent\(\)/);
  assert.doesNotMatch(source, /ensure_docker_access\(\)/);
  assert.doesNotMatch(source, /AGENT_DOCKER_NETWORK=/);
  assert.doesNotMatch(source, /LITELLM_CONTAINER=/);
  assert.doesNotMatch(source, /SKIP_HERMES_IMAGE_BUILD=/);
  assert.doesNotMatch(source, /docker network inspect/);
  assert.doesNotMatch(source, /docker container inspect/);
  assert.doesNotMatch(source, /docker image inspect "\$HERMES_IMAGE"/);
  ...
  assert.doesNotMatch(source, /require_command docker/);
  ...
  assert.match(source, /ensure_agent_python_runtime\(\) \{/);
  assert.match(source, /python-pptx python-docx openpyxl xlsxwriter pandas numpy matplotlib pillow reportlab pypdf requests beautifulsoup4 lxml markdown/);
  assert.match(source, /PIP_CONFIG_FILE="\$AGENT_PYTHON_VENV\/pip\.conf" "\$AGENT_PYTHON_VENV\/bin\/python" -m pip install --upgrade pip setuptools wheel/);
  assert.match(source, /if command -v apt-get[\s\S]*?elif command -v dnf[\s\S]*?elif command -v yum/);
  assert.match(source, /ensure_claude_code_sandbox_deps/);
  assert.match(source, /ensure_agent_python_runtime/);
  assert.doesNotMatch(source, /hermes-image\/Dockerfile/);
  ...
  assert.match(source, /Environment=MIA_CLOUD_AGENT_PYTHON_VENV=\$AGENT_PYTHON_VENV/);
  assert.match(source, /Environment=MIA_PIP_INDEX_URL=\$AGENT_PIP_INDEX_URL/);
  assert.match(source, /Environment=MIA_CLOUD_CLAUDE_CODE_SANDBOX_REQUIRED=\$CLAUDE_CODE_SANDBOX_REQUIRED/);
...
test("ssh deploy script restores data backups during install and public verification rollback", () => {
  ...
  assert.doesNotMatch(source, /command -v docker >\/dev\/null/);
  ...
  assert.match(source, /packages\+=\(bubblewrap\)/);
  assert.match(source, /packages\+=\(socat\)/);
  assert.match(source, /AGENT_PYTHON_VENV_QUOTED="\$\(shell_quote "\$AGENT_PYTHON_VENV"\)"/);
  assert.match(source, /ensure_agent_python_runtime\(\) \{/);
  assert.match(source, /python-pptx python-docx openpyxl xlsxwriter pandas numpy matplotlib pillow reportlab pypdf requests beautifulsoup4 lxml markdown/);
  assert.match(source, /PIP_CONFIG_FILE="\\\$AGENT_PYTHON_VENV\/pip\.conf" "\\\$AGENT_PYTHON_VENV\/bin\/python" -m pip install --upgrade pip setuptools wheel/);
  assert.match(source, /if command -v apt-get[\s\S]*?elif command -v dnf[\s\S]*?elif command -v yum/);
  assert.match(source, /ensure_claude_code_sandbox_deps[\s\S]*?run_as_root mkdir -p "\$BACKUP_DIR"/);
  assert.match(source, /ensure_agent_python_runtime[\s\S]*?run_as_root mkdir -p "\$BACKUP_DIR"/);
  assert.doesNotMatch(source, /AGENT_DOCKER_NETWORK=/);
  assert.doesNotMatch(source, /LITELLM_CONTAINER=/);
  assert.doesNotMatch(source, /SKIP_HERMES_IMAGE_BUILD=/);
  assert.doesNotMatch(source, /docker network inspect/);
  ...
  assert.match(source, /AGENT_MODEL_BASE_URL="\$\{MIA_CLOUD_AGENT_MODEL_BASE_URL:-http:\/\/litellm:4000\/v1\}"/);
  assert.match(source, /AGENT_MODEL_API_KEY="\$\{MIA_CLOUD_AGENT_MODEL_API_KEY:-\$\{MIA_LITELLM_API_KEY:-\}\}"/);
  assert.doesNotMatch(source, /Environment=MIA_CLOUD_AGENT_DOCKER_NETWORK=/);
  assert.match(source, /Environment=MIA_CLOUD_AGENT_MODEL_BASE_URL=\$AGENT_MODEL_BASE_URL/);
  assert.match(source, /Environment=MIA_CLOUD_AGENT_MODEL_API_KEY=\$AGENT_MODEL_API_KEY/);
  assert.doesNotMatch(source, /hermes-image\/Dockerfile/);
  assert.match(source, /Environment=MIA_CLOUD_AGENT_MODE=\$AGENT_MODE/);
  ...
  assert.match(source, /Skipping public smoke because MIA_CLOUD_TOKEN is unset/);
  assert.match(source, /cloud:prod:verify:e2e with MIA_CLOUD_TOKEN/);
