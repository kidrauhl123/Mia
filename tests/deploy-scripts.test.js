...
test("ssh deploy script restores data backups during install and public verification rollback", () => {
  ...
  assert.match(source, /if is_legacy_hermes_agent_mode; then[\s\S]*?command -v docker >\/dev\/null && \(command -v usermod >\/dev\/null \|\| test -x \/usr\/sbin\/usermod\)/);
  ...
  assert.match(source, /Environment=MIA_CLOUD_AGENT_MODEL_BASE_URL=\$AGENT_MODEL_BASE_URL/);
  assert.match(source, /Environment=MIA_CLOUD_AGENT_MODEL_API_KEY=\$AGENT_MODEL_API_KEY/);
  assert.match(source, /if is_legacy_hermes_agent && \[ -f "\$REMOTE_RELEASE_DIR\/hermes-image\/Dockerfile" \]; then/);
  assert.match(source, /Environment=MIA_CLOUD_AGENT_MODE=\$AGENT_MODE/);
  assert.match(source, /Environment=MIA_CLOUD_CLAUDE_CODE_BASE_URL=\$CLAUDE_CODE_BASE_URL/);
  assert.match(source, /Environment=MIA_CLOUD_AGENT_PYTHON_VENV=\$AGENT_PYTHON_VENV/);
  assert.match(source, /Environment=MIA_PIP_INDEX_URL=\$AGENT_PIP_INDEX_URL/);
