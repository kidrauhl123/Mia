...
- Claude Code / Codex 优先复用用户本机 CLI；桌面包同时内置固定、校验过的完整稳定运行组件。本机版本缺失或损坏时会自动使用随包版本，无需首次下载或手动启用。
...
- 桌面包携带 Rust Core 和 Claude/Codex 稳定运行组件，不携带 Hermes 或登录态；Hermes 缺失时才按需下载固定备份。
...
开发态由 Rust Core 自动准备 Claude/Codex 资源，写入当前 `Mia-Dev` 数据目录的 `managed-resources`，不会污染已安装版的用户数据；正式桌面包已内置同版本 ACP 和主 CLI，首次启动不需要 npm 下载。

### 本地 Agent 前提

Mia 会先扫描系统中的 Hermes、Claude Code 和 Codex，系统版本可用时直接复用。Claude/Codex 的 ACP 和主 CLI 随正式桌面包提供，系统版本缺失或损坏时自动切换到随包版本；Hermes 缺失时可在本机引擎区启用 Mia 稳定版，从 `https://mia.gifgif.cn/downloads/engine-backups/v1/manifest.json` 下载并校验后放进 Mia 私有目录。不会执行全局 npm 安装、不修改 PATH，也不覆盖用户 CLI：
...
当前桌面稳定资源固定为：Hermes `2026.7.7.2` / PyPI `0.18.2`（Python `3.11.13`）、Claude Code CLI `2.1.211` + ACP `0.59.0`、Codex CLI `0.144.5` + ACP `1.1.4`。Claude/Codex 随桌面包发布，Hermes 由独立备份提供。升级时需同步更新 Core pin 和对应平台的资源清单。
