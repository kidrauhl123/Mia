...
- Hermes 是上游开源 runtime，和 claude/codex 一样复用用户自己装的（从 PATH 探测），不随安装包打包。
...
Mia 不会把 Claude Code / Codex / OpenClaw CLI 打进安装包。要使用对应 Bot，推荐本机已能找到命令；如果缺失，也可以在 Mia 的本机引擎区从官方包索引触发安装：
...
Hermes 是 Mia 支持的上游开源 Agent runtime，用户按官方方式自行安装，Mia 从 PATH 探测复用（不干预安装路径），不随安装包打包。
...
Claude Code / Codex 是用户机器上的外部 CLI。OpenClaw 按最新 AionUi 的方向作为 ACP backend（`agent_type: acp`, `backend: openclaw`）对待；Mia 通过用户安装的 OpenClaw CLI 启动 `openclaw acp`，并用稳定 sessionKey 续接 OpenClaw Gateway 会话。OpenClaw Gateway 需要已配置且可连接；Mia 只探测和调用，不分发。

Hermes 也是上游 runtime：用户按官方方式自行安装，Mia 从 PATH 探测复用（`src/main/system-hermes-service.js`），不打包、不指定安装路径。云端托管 runtime 现在只保留 `cloud-claude-code`。
...
这些工具属于用户环境，版本和登录态不随 Mia 安装包锁死。Mia 可以复用已安装 CLI，也可以在用户点安装时调用官方包索引安装；它们仍不是 Mia 的打包依赖。
